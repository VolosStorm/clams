...
CLAM version 1.2.0 is now available from:

  <https://clamclient.com/bin/clam-2.1.0/>
...
To receive security and update notifications, please subscribe to:
  ...
  <https://clamclient.com/en/list/announcements/join/>

Compatibility
==============

CLAM is extensively tested on multiple operating systems using
...
CLAM should also work on most other Unix-like systems but is not
...
CLAM has supported automatically pruning the blockchain since 0.11. Pruning
...
now ZMQ can be used on Windows. Please see [this document](https://github.com/nochowderforyou/clams/blob/master/doc/zmq.md) for
...
When CLAM is out-of-sync on startup, a semi-transparent information
...
Commands sent over the JSON-RPC interface and through the `clam-cli` binary
can now use named arguments. This follows the [JSON-RPC specification](http://www.jsonrpc.org/specification)
for passing parameters by-name with an object.

`clam-cli` has been updated to support this by parsing `name=value` arguments
when the `-named` option is given.

Some examples:

    src/clam-cli -named help command="help"
    src/clam-cli -named getblockhash height=0
    src/clam-cli -named getblock blockhash=000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f
    src/clam-cli -named sendtoaddress address="(snip)" amount="1.0" subtractfeefromamount=true
...
----------------------
 ...
 - A new RPC command `getmemoryinfo` has been added which will return information
   about the memory usage of CLAM. This was added in conjunction with
...
- #8722 `bb843ad` clam-cli: More detailed error reporting (laanwj)
...
- #8830 `ef0801b` test: Add option to run clam-util-test.py manually (jnewbery)
...
- #9865 `289204f` Change clam address in RPC help message (marijnfs)
...
- #8813 `fb24d7e` clamd: Daemonize using daemon(3) (laanwj)
...
- #8837 `2108911` Allow clam-tx to parse partial transactions (jnewbery)
- #9204 `74ced54` Clarify CreateTransaction error messages (instagibbs)
- #9265 `31bcc66` clam-cli: Make error message less confusing (laanwj)
...
- #8883 `d5d4ad8` Add all standard TXO types to clam-tx (jnewbery)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/clam/).
