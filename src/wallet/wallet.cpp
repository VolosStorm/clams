...
int64_t nMinimumInputValue = DEFAULT_MININPUT;
...
void CWallet::AddToSpends(const uint256& wtxid)
{
    ...
    if (thisTx.IsCoinPoW()) // Coinbases don't spend anything!
        return;
...
void CWallet::AvailableCoins(vector<COutput>& vCoins, bool fOnlyConfirmed, const CCoinControl *coinControl, bool fIncludeZeroValue, bool fOnlyMature) const
{
    vCoins.clear();
...
CAmount CWallet::GetStake() const
{
    ...
    for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
    {
        ...
        if (pcoin->IsCoinStake() && pcoin->GetBlocksToMaturity() > 0 && pcoin->GetDepthInMainChain() > 0) {
            // LogPrintf("%s: maturing coin worth %s\n", it->first.ToString(), FormatMoney(CWallet::GetCredit(*pcoin, ISMINE_SPENDABLE)));
...
bool CWallet::FundTransaction(CMutableTransaction& tx, CAmount& nFeeRet, bool overrideEstimatedFeeRate, const CFeeRate& specificFeeRate, int& nChangePosInOut, std::string& strFailReason, bool includeWatching, bool lockUnspents, const std::set<int>& setSubtractFeeFromOutputs, bool keepReserveKey, const CTxDestination& destChange, std::string strClamSpeech)
{
    ...
    CWalletTx wtx;
    if (!CreateTransaction(vecSend, wtx, reservekey, nFeeRet, nChangePosInOut, strFailReason, &coinControl, false, false, strClamSpeech))
        return false;
...
bool CWallet::CreateTransaction(const vector<CRecipient>& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet,
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl* coinControl, bool sign, bool hasSender, std::string strClamSpeech)
{
    CAmount nValue = 0;
    ...
    if (strClamSpeech.empty() && GetBoolArg("-clamstake", DEFAULT_CLAM_STAKE))
        strClamSpeech = GetDefaultClamSpeech();

    txNew.strClamSpeech = strClamSpeech;
...
void CWallet::SearchNotaryTransactions(uint256 hash, std::vector<std::pair<std::string, int> >& vTxResults)
{
    ...
    for (int i = 0; pindexFirst && i < blockstogoback; i++) {
        ...
        CBlock block;
        if (!ReadBlockFromDisk(block, pindexFirst, chainParams.GetConsensus()))
            return;
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, const CAmount& nTotalFees, uint32_t nTimeBlock, CMutableTransaction& tx, CKey& key)
{
    ...
    BOOST_FOREACH(const PAIRTYPE(const CWalletTx*, unsigned int)& pcoin, setCoins)
    {
        ...
        for (unsigned int n=0; n<min(nSearchInterval,(int64_t)nMaxStakeSearchInterval) && !fKernelFound && pindexPrev == chainActive.Tip(); n++)
        {
            ...
            if (CheckStakeKernelHashV2(pindexPrev, nBits, blockFrom->nTime, *pcoin.first, prevoutStake, txNew.nTime - n, hashProofOfStake, bnTargetProofOfStake, fDebug))
            {
                ...
                {
                    LogPrint("stake", "CreateCoinStake : failed to parse kernel\n");
                    break;
                }
                if (whichType != TX_PUBKEY && whichType != TX_PUBKEYHASH)
                {
                    LogPrint("stake", "CreateCoinStake : no support for kernel type=%d\n", whichType);
                ...
                if (whichType == TX_PUBKEYHASH) // pay to address type
                {
                    ...
                    {
                        LogPrint("stake", "CreateCoinStake : failed to get key for kernel type=%d\n", whichType);
                ...
                if (whichType == TX_PUBKEY)
                {
                    ...
                    {
                        LogPrint("stake", "CreateCoinStake : invalid key for kernel type=%d\n", whichType);
                        break; // keys mismatch
                    }
    ...
    if (GetBoolArg("-clamstake", DEFAULT_CLAM_STAKE)) {
        if (weightedStakeSpeech.size()) {
            txNew.strClamSpeech = weightedStakeSpeech.select(hashProofOfStake.GetCheapHash());
    ...
    if (fStakeTo && stakingkeyID != staketokeyID) {
        nFirstRealOutput = 2;
        scriptStakeTo = GetScriptForDestination(staketokeyID);
        CTxOut txout(0, scriptStakeTo);
        txNew.vout.push_back(txout);
    } else {
        nFirstRealOutput = 1;
        ...
        (!nSplitSize && GetWeight(nBlockTime, (int64_t)txNew.nTime) < GetStakeSplitAge()))
    {
        ...
        if (nCredit <= nCombineLimit) {
            BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
            {
                ...
                {
                    // skip this output if ..
                    if (nCredit + pcoin.first->tx->vout[pcoin.second].nValue > nCombineLimit) // .. it causes the total to exceed the combine threshold
                        continue;

                    // skip all remaining outputs if..
                    if (txNew.vin.size() >= 100 ||                                                      // .. we already have enough inputs
                        nCredit + pcoin.first->tx->vout[pcoin.second].nValue > nBalance - nReserveBalance)  // or we have reached the reserve limit
                        break;
    ...
    if (fRewardTo) {
        scriptStakeTo = GetScriptForDestination(rewardtokeyID);
        CTxOut txout(nReward, scriptStakeTo);
        txNew.vout.push_back(txout);
    }
...

    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while(true)
            {
                ...
                BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
                        {
                                int64_t nCredit = pcoin.first->tx->vout[pcoin.second].nValue;
                ...
                // NOTE: this depends on the exact behaviour of GetMinFee
                        if (nFeeRet < DEFAULT_BLOCK_MIN_TX_FEE && nChange > 0 && nChange < CENT)
                        {
                                int64_t nMoveToFee = min(nChange, DEFAULT_BLOCK_MIN_TX_FEE - nFeeRet);
                ...
                CScript scriptNotary = GetScriptForDestination(vchPubKey.GetID());

                        txNew.vout.push_back(CTxOut(nChange, scriptNotary));
                ...
                // Sign
                        int nIn = 0;
                        BOOST_FOREACH(const PAIRTYPE(const CWalletTx*,unsigned int)& coin, setCoins)
                            if (!SignSignature(*this, *coin.first, txNew, nIn++, SIGHASH_ALL)) {
...
int CMerkleTx::GetBlocksToMaturity() const
{
    if (!(IsCoinBase()))
        return 0;
    return max(0, (Params().GetConsensus().nCoinbaseMaturity+10) - GetDepthInMainChain());
}
