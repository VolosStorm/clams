...
int64_t UpdateTime(CBlockHeader* pblock, const Consensus::CParams& consensusParams, const CBlockIndex* pindexPrev)
{
...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    */
    ...
    pblock->nTime          = std::max(pindexPrev->GetPastTimeLimit( chainparams.GetConsensus().nProtocolV2Height )+1, pblock->GetMaxTransactionTime());
