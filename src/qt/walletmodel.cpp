...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(const QString &clamspeech, WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    CAmount total = 0;
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(const QString &clamspeech, WalletModelTransaction &transaction)
{
...

    {
        ...
        Q_FOREACH(const SendCoinsRecipient &rcp, transaction.getRecipients())
        {
            ...
            else if (!rcp.message.isEmpty()) // Message from normal clam:URI (clam:x123...?message=example)
...
void WalletModel::searchNotaryTx(uint256 hash)
{
    ...
    Q_EMIT notarySearchComplete(txResults);
}
...
void WalletModel::getPetitionSupport(int nWindow)
{
    ...
    for (int i = nBlock + 1 - nWindow; i <= nBlock; i++) {
        ...
        std::set<std::string> sup = pblockindex->GetSupport(block);
        BOOST_FOREACH(const std::string &s, sup) {
            mapSupport[s]++;
        }
    }
    Q_EMIT petitionSupportRetrieved(mapSupport);
}
