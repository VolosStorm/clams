...
UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. nblocks      (numeric, required) How many blocks are generated immediately.\n"
            "2. address      (string, required) The address to send the newly generated CLAM to.\n"
...
UniValue getstakinginfo(const JSONRPCRequest& request)
{
    ...
    double dNetworkWeight = GetPoSKernelPS()/COIN;
    bool staking = nLastCoinStakeSearchInterval && nWeight;
    const Consensus::CParams& consensusParams = Params().GetConsensus();
    int64_t nTargetSpacing = consensusParams.nTargetStakeSpacing;
    uint64_t nExpectedTime = staking ? (nTargetSpacing * dNetworkWeight / nWeight) : 0;
...
UniValue estimatesmartfee(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"feerate\" : x.x,     (numeric) estimate fee-per-kilobyte (in CLAMs)\n"
