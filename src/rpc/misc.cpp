...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw runtime_error(
            ...
            "  \"mininput\": x.xxxx,         (numeric) the smallest input size recognized by the client\n"
#endif
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "\nReturn information about the given CLAM address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The CLAM address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"address\", (string) The CLAM address validated\n"
            ...
            "}\n"
            "\nExamples:\n"
            + HelpExampleCli("validateaddress", "\"xDjvN21NoFmgTqbrFsCvGDdXuaHXu4gfcX\"")
            + HelpExampleRpc("validateaddress", "\"xDjvN21NoFmgTqbrFsCvGDdXuaHXu4gfcX\"")
        );
...
#ifdef ENABLE_WALLET
        // Case 1: CLAM address and we have full public key:
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are CLAM addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) CLAM address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The CLAM address to use for the signature.\n"
            ...
            "\nAs json rpc\n"
            + HelpExampleRpc("verifymessage", "\"xLVL9XcEK3YcwU9UdqPp5nSRAvamt1B94e\", \"signature\", \"my message\"")
        );
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw runtime_error(
            ...
            "clam-cli and the GUI. There is no server-side difference."
        );
