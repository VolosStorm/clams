...
static bool CheckStakeKernelHashV1(unsigned int nBits, const CBlock& blockFrom, unsigned int nTxPrevOffset, const CTransaction& txPrev, const COutPoint& prevout, unsigned int nTimeTx, uint256& hashProofOfStake, uint256& targetProofOfStake, bool fPrintProofOfStake)
{
    ...
    arith_uint256(nValueIn) * arith_uint256((int64_t)txPrev.nTime, (int64_t)nTimeTx) / COIN / (24 * 60 * 60)

    arith_uint256 bnCoinDayWeight = arith_uint256(nValueIn) * arith_uint256((int64_t)txPrev.nTime, (int64_t)nTimeTx) / COIN / (24 * 60 * 60)
    targetProofOfStake = arith_uint256(bnCoinDayWeight * bnTargetPerCoinDay);
    ...
    if (arith_uint256(hashProofOfStake) > bnCoinDayWeight * bnTargetPerCoinDay)
        return false;
...
static bool CheckStakeKernelHashV2(CBlockIndex* pindexPrev, unsigned int nBits, unsigned int nTimeBlockFrom, const CTransaction& txPrev, const COutPoint& prevout, unsigned int nTimeTx, uint256& hashProofOfStake, uint256& targetProofOfStake, bool fPrintProofOfStake)
{
    ...
    targetProofOfStake = arith_uint256(bnTarget);
    ...
    LogPrint("stake", "[STAKE] PASS: hash %64s\n", arith_uint256(hashProofOfStake).GetHex());
...
bool CheckProofOfStake(CBlockIndex* pindexPrev, const CTransaction& tx, unsigned int nBits, uint256& hashProofOfStake, uint256& targetProofOfStake)
{
    ...
    // get positon of tx on disk for v1 of kernal stake check
    CDiskTxPos pos(blockFrom->GetBlockPos(), GetSizeOfCompactSize(block.vtx.size()));
    ...
    if (!CheckStakeKernelHash(pindexPrev, nBits, block, pos.nTxOffset, txPrev, txin.prevout, tx.nTime, hashProofOfStake, targetProofOfStake, fDebug))
